...
public class DockerMySQLTest {
    ...
    static Connection connectToDatabase() throws InterruptedException {
        ...
        for (;;) {
            try {
				...
				System.out.println("Wystąpil blad polaczenia. Ponawianie za 10 sek. Szczegoly ponizej.");
    ...
    static String databaseInit(Connection conn, Statement stmt) {
        try {
            ...
            ResultSet metaData = meta.getSchemas();
            metaData = meta.getTables(null, null, "users", null);
            while (metaData.next()) {
                System.out.print("Czyszcze istniejaca tabele...");
                stmt.executeUpdate("DROP TABLE IF EXISTS " + metaData.getString(3));
            }
            System.out.print("Tworze nowa tabele users...");
            ...
            System.out.print("Dodaje uzytkownikow do tabeli...");
